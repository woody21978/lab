window.onload = () => {
  console.log('Вызов функций: send(\'user\',\'message\')');
  console.log('users: \'cat\', \'dog\'');
...
function send(user,message) {
  if (user === '' || message === '') {
    console.log('имя usera или message является пустым значением :(');
    return;
  }
  let content = document.querySelector('.dialog'),
    wrap = document.querySelector('.wrap'), 
  ...
  if (user === 'cat') {
    ...
    console.log('Fail... Причина: user не существует');
    return;
  }
